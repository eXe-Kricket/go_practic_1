...
func main() {
	...
	for {
		...
		if memTotal > 0 {
			...
			if usage > 0.8 {
				// Округляем ВНИЗ (отбрасываем дробную часть)
		...
		if diskTotal > 0 {
			...
			if usage > 0.9 {
				// Свободное место в Мб, округляем ВНИЗ
		...
		if netTotal > 0 {
			...
			if usage > 0.9 {
				// Свободная полоса в Мбит/с
				// По спецификации: байт/с -> Мбит/с = байт/с / 125000
				freeMbits := (netTotal - netUsed) / 125000
				// Округляем ВНИЗ
